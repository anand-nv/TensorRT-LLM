...
class T5TTSDecoderLayer(Module):
    ...
    def __init__(self,
        ...
        # e.g. BART q_scaling = 1.f, T5 q_scaling = 1.f/sqrt(head_size)
        print(
            f"{local_layer_idx=}, {hidden_size=}, {head_size=}, {num_attention_heads=}, {num_kv_heads=}, {max_position_embeddings=}"
        )
        ...
        if use_cache:
            ...
            if not self.has_encoder_input_layernorm:
                ...
                if self.cache_cross_attention_memory is not None:
                    ...
                    encoder_memory = self.cross_attention_memory_layernorm(
                        encoder_output)
...
class T5TTSDecoderModel(PretrainedModel):
    ...
    def __init__(self, config: PretrainedConfig):
        ...
        self.has_position_embedding = self.config.has_position_embedding
        self.decoder_layers_mapping = {}
        if self.config.text_context_mapping is not None:
            self.num_encoders = len(self.config.text_context_mapping)
        else:
            self.num_encoders = 1
            self.decoder_layers_mapping = {1: range(self.total_num_layers)}
        ...
        if self.mapping.is_first_pp_rank():
            ...
            self.audio_embeddings = ModuleList([
                Embedding(self.num_audio_tokens_per_codebook,
                          self.audio_embedding_dim) for idx in audio_emb_range
            ])
    ...
    def check_config(self, config: PretrainedConfig):
        config.set_if_not_exist('has_position_embedding', False)
        config.set_if_not_exist('text_context_mapping', None)
        config.set_if_not_exist('type_vocab_size', None)
        config.set_if_not_exist('rescale_before_lm_head', False)
        config.set_if_not_exist('layernorm_type', LayerNormType.LayerNorm)
        config.set_if_not_exist('layernorm_position',
    ...
    def forward(self,
        ...
        if self.mapping.is_first_pp_rank():
            ...
            if additional_decoder_mask is not None and additional_decoder_input is not None:
                ...
                audio_embeddings = concat(additional_decoder_input,
                                          audio_embeddings)
            print(f"{decoder_input_ids.shape=} {audio_embeddings.shape=}")

            hidden_states = self.embedding(audio_embeddings,
                                           position_ids=position_ids)
    ...
    def prepare_inputs(self,
        ...
        if not use_gpt_attention_plugin:
            ...
            cross_attention_mask = Tensor(
                ...
                dim_range=OrderedDict([
                    ('batch_size_beam_width', [bb_range]),
                    ('query_len', [[1, 1, max_decoder_input_len]]),
