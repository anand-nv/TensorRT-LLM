...
class T5TTSGenerationSession(GenerationSession):
    ...
    def __init__(
        ...
        self.deterministic_seed = torch.Generator(
            device=self.device).manual_seed(2147483647)
    ...
    def _prepare_attention_mask(input_ids: torch.Tensor,
        ...
        if input_ids is not None and is_pad_id_in_inputs:
            ...
            return torch.ones([input_ids.shape[0], input_ids.shape[-1]],
                              dtype=torch.int32,
                              device=input_ids.device)
    ...
    def _prepare_context_inputs(self, batch_size, context_lengths,
        ...
        if use_gpt_attention_plugin:
            ...
            if self.has_attn_layers:
                input_ids = kwargs.pop('input_ids')
                pad_id = kwargs.pop('pad_id', None)
                attention_mask = self._prepare_attention_mask(input_ids, pad_id)
                position_ids = attention_mask.long().cumsum(-1) - 1
                position_ids.masked_fill_(attention_mask == 0, 1)
                position_ids = position_ids.int()
                ret['attention_mask'] = attention_mask
    ...
    def sample_codes_from_logits(self, logits, temperature=0.7, topk=80):
        ...
        for idx in range(self._model_config.num_audio_codebooks):
            ...
            codebook_logits_topk = torch.topk(codebook_logits, topk,
                                              dim=-1)[0]  # (B, topk)
            indices_to_remove = codebook_logits < codebook_logits_topk[:, -1].unsqueeze(
                -1)  # (B, num_tokens_per_codebook)
            codebook_logits_rescored = codebook_logits.clone()
            codebook_logits_rescored[indices_to_remove] = float('-inf')

            codebook_probs = torch.softmax(
                codebook_logits / temperature,
                dim=-1)  # (B, num_tokens_per_codebook)
            codebook_preds = torch.multinomial(
                codebook_probs, 1, generator=self.deterministic_seed)  # (B, 1)
            all_preds.append(codebook_preds)
        all_preds = torch.cat(all_preds, dim=1).long()  # (B, num_codebooks)
        return all_preds

    def decode_logits(self, batch_size, logits, step):
        audio_codes_next = self.sample_codes_from_logits(logits)
        all_codes_next_argmax = self.sample_codes_from_logits(logits,
                                                              temperature=0.01)
        ...
        for item_idx in range(batch_size):
            if item_idx not in self.end_indices:
                ...
                if (pred_token == self._model_config.eos_token_id) or (
                        pred_token_multinomial
                        == self._model_config.eos_token_id):
                    print("End detected for item {} at timestep {}".format(
                        item_idx, step))
                    self.end_indices[item_idx] = step

        self.all_predictions.append(audio_codes_next)
        if step > 0:
            self.output_ids[:, :, step] = audio_codes_next

        should_stop = torch.as_tensor([len(self.end_indices) == batch_size],
                                      dtype=torch.bool)
        ...
        if self.mapping.is_last_pp_rank():
            ...
            if self.is_redrafter_mode:
                ...
                if self.is_medusa_mode:
                    ...
                    should_stop = self.decode_logits(batch_size=batch_size,
                                                     logits=logits,
                                                     step=step)
            ...
            if self.is_redrafter_mode:
                ...
                input_ids_shape = (batch_size * beam_width *
                                   (self.num_draft_tokens +
                                    1), ) if self.remove_input_padding else (
                                        batch_size * beam_width,
                                        self._model_config.num_audio_codebooks,
                                        self.num_draft_tokens + 1)
            if self.is_redrafter_mode:
                add_tensor_with_shape(self.buffer['flat_tokens'], 'input_ids',
                                      input_ids_shape)
    ...
    def __setup_decoder(self, input_ids: torch.Tensor,
        ...
        if self.is_redrafter_mode:
            ...
            self.new_tokens = torch.zeros(
                [batch_size, self._model_config.num_audio_codebooks, 1],
                dtype=torch.int32,
                device=self.device)
        if scfg.num_beams > 1 or scfg.output_cum_log_probs:
            self.cum_log_probs = torch.full((batch_size, scfg.num_beams),
