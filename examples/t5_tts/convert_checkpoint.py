...
def parse_model_config(args, ):
    ...
    config["encoder"]['vocab_size'] = "106339"
    ...
    config["encoder"]['d_kv'] = f"{int(768/12)}" # config["encoder"]['d_model'] /config["encoder"]["num_heads"]
    ...
    #config["decoder"]['d_kv'] = f"{768/12}" # config["decoder"]['d_model'] /config["decoder"]["num_heads"]
    ...
    def parse_t5_config_by_component(config, component, args):
        ...
        component_config.hidden_act = config.get(component, 'dense_act_fn', fallback="gelu")
        component_config.gated_act = config.getboolean(component,
                                                       'is_gated_act', fallback=True)
        ...
        if component == 'encoder':
            ...
            component_config.eos_token_id = config.getint(
                'decoder', 'eos_token_id', fallback=2048-1)
            bos_token_id = config.get('decoder', 'bos_token_id', fallback=2048-2)
            ...
            component_config.pad_token_id = config.getint(
                'decoder', 'pad_token_id', fallback=0)
...
def convert_t5tts_encoder(config, model_dict, quant_algo: str = None,):

    weights={}
    weights['embedding.vocab_embedding.weight'] = model_dict['text_embedding.weight'].contiguous()
    weights['embedding.position_embedding.weight'] = model_dict['t5_encoder.position_embeddings.weight'].contiguous()
    ...
    for i in range(num_layers):
        ...
        weights[f'encoder_layers.{i}.ff_layernorm.weight'] = model_dict[
            f't5_encoder.layers.{i}.norm_pos_ff.weight'].contiguous()
        weights[f'encoder_layers.{i}.ff.proj.weight'] = model_dict[
            f't5_encoder.layers.{i}.pos_ff.proj.conv.weight'].unsqueeze(3).contiguous()
        weights[f'encoder_layers.{i}.ff.o_net.weight'] = model_dict[
            f't5_encoder.layers.{i}.pos_ff.o_net.conv.weight'].unsqueeze(3).contiguous()
...
def convert_t5tts_decoder(config, model_dict, quant_algo: str = None,):
    ...
    weights['embedding.position_embedding.weight'] = model_dict['t5_decoder.position_embeddings.weight'].contiguous()

    num_layers = config.n_layer
    for i in range(num_layers):
        weights[f'decoder_layers.{i}.self_attention_layernorm.weight'] = model_dict[
        ...
        weights[f'decoder_layers.{i}.cross_attention_layernorm.weight'] = model_dict[
...
def convert_checkpoint(args, model):
    ...
    def convert_and_save(component: str = "encoder", ):
        ...
        safetensors.torch.save_file(weights, os.path.join(component_save_dir,
                                        f'rank0.safetensors'))
