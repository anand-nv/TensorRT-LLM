...
def parse_model_config(args, ):
    ...
    def parse_t5_config_by_component(config, component, args):
        ...
        if component == 'encoder':
            ...
            component_config.vocab_sizes = [vocab_size // num_vocabs
                                            ] * num_vocabs
...
def convert_t5tts_decoder(
    ...
    weights['lm_head.bias'] = model_dict['final_proj.bias'].clone().contiguous()
    ...
    weights[f'embedding.vocab_embedding.weight'] = torch.cat(
        [
            model_dict[f'audio_embeddings.{i}.weight']
...
def convert_checkpoint(args, model):
    ...
    tllm_decoder_config = {
        ...
        'cross_attention': True,  #  this has to be provided explicitly
    }
    for additional_setting in additional_settings:
