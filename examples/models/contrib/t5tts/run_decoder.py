...
def main():
    ...
    encoder_encodings = torch.tensor(np.load("/code/tensorrt_llm/encoder_outputs/encoder_outputs_0.npy")).to(torch.float16)
    print(f"Encoder input tokens: {str(encoder_encodings.shape)}")
    ...
    for run_idx in range(1):
        with torch.no_grad():
            outputs = runner.generate(
                batch_input_ids=[decoder_input_ids] * 4,
                encoder_input_features=[encoder_encodings] * 4,
                max_new_tokens=440,
                end_id=2017,
                pad_id=2017,
                temperature=0.6,
